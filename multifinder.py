...
class CdnDetect:
    ...
    def get_http_header(self, domain, ip):
        try:
            # since some website may focus on one http protocal, so we crawl both http and https
        ...
        try:
            ...
            http_header = ""
        # print(http_header)
        return https_header + http_header
    ...
    def http_finger(self, header=None):
        ...
        patterns = json.load(open("pattern.json", 'r'))
        regex = '''(%s)\\s*(:)\\s*(%s)\\s*,?'''
        cdns = []
        key_http_header = ""
